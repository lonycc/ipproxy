...
def read_csv(fpath):
    o = gzip.open if fpath.endswith(".gz") else codecs.open
    with o(fpath, 'rb', encoding='utf-8') as f:
        csvreader = csv.DictReader(f, restval=0, delimiter=",", quotechar="\"", quoting=csv.QUOTE_MINIMAL)
...
def write_csv(ip_avaliable):
    ...
    with open(output, "w") as fw:
        w = csv.DictWriter(fw, fieldnames=header, restval="", extrasaction="ignore", delimiter=",", quotechar="\"", quoting=csv.QUOTE_MINIMAL)
        w.writeheader()
